...
def NLL(X, y, W, reg=0.0):
	...
	"*** YOUR CODE HERE ***"
	#From the formula for log likelihood
	mew = sigmoid(X @ W)
	a = np.multiply(y, np.log(mew))
	b= np.multiply((1. - y), np.log(1. - mew))
	nll = -sum(a + b) + reg / 2 * np.linalg.norm(W) ** 2
...
def newton_step(X, y, W, reg=0.0):
	...
	Hess = X.T @ diag@X +reg *np.eye(X.shape[1])
	d = np.linalg.solve(Hess,g)
...
def newton_method(X, y, reg=0.0, eps=1e-6, max_iter=20, print_freq=5):
	...
	while iter_num < max_iter and np.linalg.norm(step) > eps:
		...
		"*** YOUR CODE HERE ***"
		#similar to above but use newton step
		nll = NLL(X, y, W,reg)
		#For debugging
		if np.isnan(nll):
			break
...
def get_description(X, y, W):
	...
	#With the help from solutions to the assignment
	m, n = X.shape
	y_pred = predict(X, W)
...
def plot_description(X_train, y_train, X_test, y_test):
	...
	"*** YOUR CODE HERE ***"
	#With the help from the solutions to the assignment
