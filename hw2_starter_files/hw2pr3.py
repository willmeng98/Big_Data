...
def linreg(X, y, reg=0.0):
	...
	eye = np.eye(X.shape[1]) #Creates the identity matrix
	eye[0, 0] = 0 #The first bia term remains unchanged
	W_opt = np.linalg.solve(X.T @ X + reg * eye, X.T @ y) #Solving for W_opt
	return W_opt
...
def find_RMSE(W, X, y):
	...
	"*** YOUR CODE HERE ***"
	m = X.shape[0] #number of rows for X
	y_pred = predict(W, X) #Predict y using given weights and X values
	diff = y - y_pred #compute the singular cost
	RMSE = np.sqrt(np.linalg.norm(diff, 2) ** 2 / m) #squre root of mean error
...
def RMSE_vs_lambda(X_train, y_train, X_val, y_val):
	...
	W_list = []
	"*** YOUR CODE HERE ***"
	reg_list = np.random.uniform(0, 150, 150) #generate random using uniform dist.
	reg_list.sort() #sorts entries
	W_list =[linreg(X_train, y_train, reg = lb) for lb in reg_list] #reg parameters
	for i in range(len(reg_list)):
		W_opt = W_list[i]
		RMSE_list.append(find_RMSE(W_opt,X_val,y_val)) #finds RMSE for each one
	...
	opt_lambda_index = np.argmin(RMSE_list) #minimizes RMSE
