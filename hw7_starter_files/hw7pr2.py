...
def find_cost(X, y, W , reg):
	...
	error = X @ W - y #find the error
	error = float(error.T @error) #squared
	m = len(y)
	return (error + reg * np.abs(W).sum()) / m #mean
...
def find_grad(X, y, W, reg=0.0):
	...
	"*** YOUR CODE HERE ***"
	m = X.shape[0] #determines m
	grad = X.T @ (X @ W - y) / m #using our equation for gradient
